...
get_trade_stats_sum <- function(trade_dat) {
    nc <- length(trade_dat$P0)
    Ptot <- sum(trade_dat$P0)
    Rtot <- sum(trade_dat$R0)
    nlinks <- sum(trade_dat$E0 > 0)
    totflow <- sum(trade_dat$E0)
    RP_ratio <- Rtot / Ptot
...
###############################
#needed as input for 1 country and multi country cascade scenarios
...
# Runs sim_cascade starting with country c_init losing a fraction a_init of P
...
#########################################
### ONE SHOCK, EACH COUNTRY AT A TIME ###
#########################################
...
#Runs sim_cascade for multiple countries

sim_mc <- function(a_init, P0, R0, E0, cfrac, asym, kmax, amin, anim_out) {
    nc <- length(P0)
    cnames <- names(P0)
    if (!all(names(a_init) %in% cnames)) {
    ...
    dP <- rep(0, nc)
    names(dP) <- cnames
    dP[names(a_init)] <- -a_init * P0[names(a_init)]
    sim_cascade(dP, P0, R0, E0, cfrac, asym, kmax, amin, anim_out)
}

################################################
### MULTIPLE COUNTRIES, MULTIPLE SHOCK SIZES ###
################################################

# Runs sim_cascade for multiple countries and multiple scenarios

sim_mc_multi <- function(scenarios, P0, R0, E0, cfrac, asym, kmax, amin) {
    cnames <- names(P0)
    nc <- length(P0)
    nsim <- length(scenarios)
    res_multi <- lapply(scenarios, sim_mc, P0, R0, E0, cfrac, asym, kmax, amin, anim_out = FALSE)
    list(scenarios = scenarios, P0 = P0, R0 = R0, E0 = E0,
         dR = matrix(vapply(res_multi, `[[`, "dR", FUN.VALUE = rep(0, nc)),
                     nrow = nc, dimnames = list(cnames, NULL)),
         dC = matrix(vapply(res_multi, `[[`, "dC", FUN.VALUE = rep(0, nc)),
                     nrow = nc, dimnames = list(cnames, NULL)),                           
         dE = array(vapply(res_multi, `[[`, "dE", FUN.VALUE = rep(0, nc*nc)),
                    dim = c(nc, nc, nsim), dimnames = list(cnames, cnames, NULL)))
}
...
#  ana a logical vector indicating shocked countries (that can't export any more)
...
# This function calculates different summary stats for one sim_res_multi
#  (i.e. outcome of sim_mc_multi) and arranges them in a list
get_stats_mcmulti <- function(sim_result_mc) {
  # Initial net supply by country
  S0 <- with(sim_result_mc, P0 + colSums(E0) - rowSums(E0))
  # Magnitude of initial shock by sim
  nsim <- ncol(sim_result_mc$dR)
  # this returns a vector of length nsim where each element is the 
  # total dP (summed over initially affected countries) in each sim.
  dS0 <- vapply(sim_result_mc$scenarios, function(x) sum(x * sim_result_mc$P0[names(x)]), 0)
  
  # Values of dR, dC, dE relative to initial shock
  dRrel <- sweep(sim_result_mc$dR, 2, dS0, "/")
  dCrel <- sweep(sim_result_mc$dC, 2, dS0, "/")
  dErel <- sweep(sim_result_mc$dE, 3, dS0, "/")
  dSrel <- dRrel + dCrel  
  # Values of dC and dS relative to initial net supply
  dC_S0 <- sweep(sim_result_mc$dC, 1, S0, "/")
  dS_S0 <- sweep(sim_result_mc$dR, 1, S0, "/") + dC_S0
  
  # Statistics by simulation
  depth_by_sim <- vapply(1:nsim, 
                         function(i) cascade_depth(names(sim_result_mc$scenarios[[i]]), sim_result_mc$dE[, , i]), 0)
...
# This function calculates different summary stats for one sim_res_multi
#  (i.e. outcome of sim_mc_multi) and arranges them in a list
get_stats_multiyr <- function(result_multiRcoef) {
  # Initial net supply by country
  S0 <- result_multiRcoef$`2000`$P0 + colSums(result_multiRcoef$`2000`$E0) - rowSums(result_multiRcoef$`2000`$E0)
  # Magnitude of initial shock by sim
  nsim <- ncol(result_multiRcoef$`2000`$dR)
  # this returns a vector of length nsim where each element is the 
  # total dP (summed over initially affected countries) in each sim.
  dS0 <- vapply(result_multiRcoef$scenarios, function(x) sum(x * result_multiRcoef$P0[names(x)]), 0)
  
  # Values of dR, dC, dE relative to initial shock
  dRrel <- sweep(result_multiRcoef$dR, 2, dS0, "/")
  dCrel <- sweep(result_multiRcoef$dC, 2, dS0, "/")
  dErel <- sweep(result_multiRcoef$dE, 3, dS0, "/")
  dSrel <- dRrel + dCrel  
  # Values of dC and dS relative to initial net supply
  dC_S0 <- sweep(result_multiRcoef$dC, 1, S0, "/")
  dS_S0 <- sweep(result_multiRcoef$dR, 1, S0, "/") + dC_S0
  
  # Statistics by simulation
  depth_by_sim <- vapply(1:nsim, 
                         function(i) cascade_depth(names(result_multiRcoef$scenarios[[i]]), result_multiRcoef$dE[, , i]), 0)
